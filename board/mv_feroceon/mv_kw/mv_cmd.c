...
extern char diag_cmd[][15];
extern void diag_get_mem_detail(unsigned int **mem_test_start_offset, 
...
int mv_diag (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
   ...
   diag_get_mem_detail(&mem_test_start_offset, &mem_test_end_offset);
	...
	if (argc == 1)
	{
		diag_func_ptr = diag_sequence;

		printf("Available tests are: \n");
		for (test_no = 0; diag_func_ptr[test_no] != NULL; test_no++)
			printf(" %s\n", diag_cmd[test_no]);
	...
	else if ( (argc > 1) && (strcmp(argv[1], "all")) )
	{
		...
		for (test_no = 0; diag_func_ptr[test_no] != NULL; test_no++)
			if (!strcmp(diag_cmd[test_no], argv[1]))
				break;

		if (diag_func_ptr[test_no] == NULL)
		{
