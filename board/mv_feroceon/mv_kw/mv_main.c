...
void misc_init_r_env(void){
	...
	{
		unsigned long mtd1Size=0;
		char *mtd1Size_buf;

		mtd1Size_buf = getenv("mtd1Size");

		if((!mtd1Size_buf) || ((mtd1Size = (simple_strtoul(mtd1Size_buf, NULL, 16))) == 0))
			mtd1Size = 0x300000;

		mtd1Size = simple_strtoul(mtd1Size_buf, NULL, 16);
		
		sprintf(buff,"console=ttyS0,115200 mtdparts=nand_mtd:0x%x@0(uboot)ro,0x%x@0x%x(uImage),0x%x@0x%x(rootfs)rw",
			 0x100000, mtd1Size, 0x100000, (nand_info[0].size - 0x100000 - mtd1Size), (0x100000+mtd1Size));
/*
		env = getenv("console");
		if(!env)
*/
...
int mv_set_power_scheme(void)
{
	...
	if (devId == MV_6180_DEV_ID || boardId == RD_88F6281A_PCAC_ID || boardId == SHEEVA_PLUG_ID)
	{
		/* Sata power down */
    ...
    if (boardId == RD_88F6281A_ID || boardId == SHEEVA_PLUG_ID)
    {
...
void recoveryHandle(void)
{
    ...
    sprintf(cmd,"setenv bootargs $(console) root=/dev/ram0 rootfstype=squashfs initrd=0x%x,0x%x ramdisk_size=%d recovery=%s serverip=%d.%d.%d.%d; bootm 0x%x;", 
            imagAddr + mtd1Size, (imagSize - mtd1Size), (imagSize - mtd1Size)/1024, ip, (NetServerIP & 0xFF), ((NetServerIP >> 8) & 0xFF), ((NetServerIP >> 16) & 0xFF), ((NetServerIP >> 24) & 0xFF),imagAddr);
